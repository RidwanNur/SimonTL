                ...
                <div class="col-md-12">
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-body">
                                @php
                                    ...
                                    $fileUrl =
                                        $link_fileLaporan->link_file &&
                                        Storage::disk('public')->exists($link_fileLaporan->link_file)
                                            ? Storage::url($link_fileLaporan->link_file)
                                            : null;

                                    // nama file (truncated nanti di UI)
                                    $fileName = $link_fileLaporan->link_file
                                        ? basename($link_fileLaporan->link_file)
                                        : null;
                                @endphp
                                        ...
                                        @if ($fileUrl)
                                            <div class="btn-group">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end">
                                                    <li><a class="dropdown-item" href="{{ $fileUrl }}"
                                                            target="_blank">Lihat</a></li>
                                                    <li><a class="dropdown-item" href="{{ $fileUrl }}"
                                                            download>Unduh</a></li>
                      ...
                      data-copy="{{ $fileUrl }}" onclick="copyFileUrl(this)">Salin tautan</button></li> --}}
                                ...
                                @role('pegawai')
                                    @if ($report)
                                        <form action="{{ route('pegawai.updateTL', $report->id) }}" method="POST"
                                            ...
                                            <div class="row g-3">
                                                ...
                                                <div class="col-md-4">
                                                    ...
                                                    <input type="text" name="phone_number_teamlead" class="form-control"
                                                        value="{{ old('phone_number_teamlead', $report->phone_number_teamlead) }}"
                                                        readonly>
                                                </div>

                                                <div class="col-md-4">
                                                    <label class="form-label">Deadline Laporan</label>
                                                    <input type="date" name="report_dateline" class="form-control"
                                                        {{-- value="{{ old('report_dateline', optional($report->report_dateline)->format('Y-m-d')) }}"> --}}
                                                        value="{{ old('report_dateline', $report->report_dateline) }}">
                                                </div>
                                                ...
                                                <div class="col-md-4">
                                                    <div class="d-flex justify-content-between align-items-center">
                                                        ...
                                                        @if ($report->sending_evidence && Storage::disk('public')->exists($report->sending_evidence))
                                                            <a href="{{ Storage::url($report->sending_evidence) }}"
                                                                target="_blank" class="small text-decoration-none">
                                                                <svg width="14" height="14" viewBox="0 0 24 24"
                                                                    fill="none" class="me-1">
                                                                    <path
                                                                        d="M14 2v6h6M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6z"
                                            ...
                                            <div class="d-flex justify-content-end mt-3 gap-3">
                                                ...
                                                {{-- @method('POST')
                                                <button type="submit" class="btn btn-primary" id="btnKirimNotif"
                                                    formaction="{{ route('pegawai.sendTL', $report->id) }}"
                                ...
                                @role('opd')
                                    @if ($report)
                                        <form action="{{ route('opd.updateTL', $report->id) }}" method="POST"
                                            class="mt-2" enctype="multipart/form-data">
                                            @csrf
                                            @method('PUT')
                                            ...
                                            <div class="row g-3">
                                                ...
                                                <div class="col-md-4">
                                                    ...
                                                    <input type="file" name="sending_evidence" class="form-control"
                                                        accept=".pdf,.doc,.docx,.xls,.xlsx,.png,.jpg,.jpeg" disabled>
                                                </div>
                                                ...
                                                <div class="col-12">
                                                    <div class="row g-3 align-items-start">
                                                        ...
                                                        {{-- File Lampiran --}}
                                                        <div class="col-md-6">
                                                            <label class="form-label mb-1">Dokumen Tindak Lanjut</label>
                                                            <div class="input-group">
                                                                <input type="file" name="tl_document" id="tl_document"
                                                                    class="form-control @error('tl_document') is-invalid @enderror"
                                                                    accept=".pdf,.doc,.docx,.xls,.xlsx,.png,.jpg,.jpeg">
                                                                @if (!empty($report->tl_document) && Storage::disk('public')->exists($report->tl_document))
                                                                    <a class="btn btn-outline-secondary"
                                                                        href="{{ Storage::url($report->tl_document) }}"
                                                                        target="_blank">Buka</a>
                                                                @endif
                                                            </div>
                                                            <div class="form-text">Format: PDF/DOC/XLS/IMG • Maks 5MB</div>
                                                            @error('tl_document')
                                                        ...
                                                        {{-- Keterangan (Textarea) --}}
                                                        <div class="col-md-6">
                                                            <label class="form-label mb-1">Keterangan Tindak Lanjut</label>
                                                            <textarea name="tl_description" id="tl_description" rows="4"
                                                                class="form-control @error('tl_description') is-invalid @enderror"
                                                                placeholder="Tuliskan keterangan hasil Tindak Lanjut">{{ old('tl_description', $report->tl_description ?? '') }}</textarea>
                                            ...
                                            <div class="d-flex justify-content-end mt-5 gap-5">
                                                <button type="submit" class="btn btn-success">Simpan</button>
                                            </div>
                                        </form>
                                    @else
